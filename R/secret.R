...
new_secret <- function(x, secrets = NULL, locked = TRUE, class = NULL, ...) {
  ...
  new_rcrd(
    ...
    ...,
    class = append(std_class(class), 'essence_secret')
  )
}
...
validate_secret <- function(x) {
  ...
  # Check class
  if (!rlang::inherits_all(x, c('essence_secret', 'vctrs_rcrd'))) {
    rlang::abort(paste(arg, 'must inherit from `essence_secret`'))
  }
...
#'
#' @export
format.essence_secret <- function(x, ...) {
  values <- scrt_x(x)
...
#'
#' @export
obj_str_footer.essence_secret <- function(x, ...) {
  cat(' locked:', scrt_locked(x))
}

#' @rdname secret-methods
#'
#' @export
names.essence_secret <- function(x) scrt_names(x)

#' @rdname secret-methods
#'
#' @export
`names<-.essence_secret` <- function(x, value) scrt_set_names(x, value)

#' @rdname secret-methods
#'
#' @export
vec_ptype_abbr.essence_secret <- function(x, ...) 'scrt'

#' @rdname secret-methods
#'
#' @export
vec_ptype_full.essence_secret <- function(x, ...) 'secret'

#' @rdname secret-methods
#'
#' @export
vec_ptype2.essence_secret.essence_secret <- function(x, y, ...) secret()

#' @rdname secret-methods
#'
#' @export
vec_ptype2.essence_secret.character <- function(x, y, ...) secret()

#' @rdname secret-methods
#'
#' @export
vec_ptype2.character.essence_secret <- function(x, y, ...) secret()

#' @rdname secret-methods
#'
#' @export
vec_cast.essence_secret.nbscar_secret <- function(x, to, ...) x

#' @rdname secret-methods
#'
#' @export
vec_cast.essence_secret.character <- function(x, to, ...) secret(x)

#' @rdname secret-methods
#'
#' @export
vec_cast.character.essence_secret <- function(x, to, ...) {
  arg <- rlang::expr_label(rlang::enexpr(x))
  if (scrt_locked(x)) {
    rlang::warn(paste(
...
#'
#' @export
`$.essence_secret` <- function(x, i) x[i]
